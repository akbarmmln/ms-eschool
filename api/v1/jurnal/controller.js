...
exports.getListJurnal = async function (req, res) {
  try {
    ...
    if (dari && sampai) {
      ...
      let queryCount = `SELECT COUNT(*) as count FROM adr_jurnal_mengajar
        WHERE id_guru = :id_guru_ AND DATE(tanggal_jurnal) BETWEEN :dari_ AND :sampai_ AND is_deleted = '0'`

      let queryData = `SELECT jm.id, jm.tanggal_jurnal, jm.jam_mulai, jm.jam_selesai, jm.materi,
